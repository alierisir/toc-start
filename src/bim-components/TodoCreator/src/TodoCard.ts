...
export class TodoCard extends OBC.SimpleUIComponent {
  ...
  set task(value: string) {
    const taskElement = this.getInnerElement("task") as HTMLParagraphElement;
    taskElement.textContent = value;
  }
  set deadline(value: Date) {
    const deadlineElement = this.getInnerElement("deadline") as HTMLParagraphElement;
    deadlineElement.textContent = value.toDateString();
  }
      ...
      <div style="display:flex;justify-content: space-between;align-items: center">
        ...
        <div>
          <div style="opacity: 0.6; font-size: 0.7rem; display: flex; column-gap: 15px">
            <p id="deadline">DATE GOES HERE</p>
            <p id="priority">PRIORITY GOES HERE</p>
          </div>
          <p id="task">TASK GOES HERE</p>
        </div>
      </div>
